...
def seeding(nn_index1,nn_index2,x1,x2,topk,match_score,confbar,nms_radius,use_mc=True,test=True):
    ...
    mask_not_local_max=((match_score.unsqueeze(-1)>=match_score.unsqueeze(-2))|nms_mask).float()
    mask_not_local_max=1.0 - mask_not_local_max.min(dim=-1).values
    match_score[mask_not_local_max==1.0] = -1
 
    #confidence bar
    match_score[match_score<confbar]=-1
    mask_survive=match_score>0
    # if test:
        #sum_result = mask_survive.sum(dim=1)
        #print(f"sum_result shape: {sum_result.shape}")
    #topk = torch.tensor(topk,dtype=torch.float32)
    #topk = torch.minimum(mask_survive.sum(dim=1)[0] + 2, topk)
    #将topk重新从tesnor转为int
    #topk = topk.int()
    #     #将topk转化为torch.tensor
...
class matcher(nn.Module):
    ...
    def forward(self,in_x1,in_x2,in_desc1,in_desc2,test_mode=True):
        x1, x2, desc1, desc2 = in_x1[:,:,:2], in_x2[:,:,:2], in_desc1, in_desc2
        desc1, desc2 = torch.nn.functional.normalize(desc1,dim=-1), torch.nn.functional.normalize(desc2,dim=-1)
        # if test_mode:
        encode_x1,encode_x2=in_x1,in_x2
        # else:
